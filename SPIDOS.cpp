...
void display_title() //displays the games title screen
{
	...
	textcolor(13 + BLINK);
	gotoxy(x, y++);
	cprintf(" ����� ����Ŀ  �%c%c%c%c%c%c%c� ����Ŀ ����,  �����  ����Ŀ  ������  ", 4, 4, 4, 4, 4, 4, 4);
	gotoxy(x, y++);
	cprintf("   �   �    �  ��%c   %c�� �    � �    \\ �      �    �  �      ", 4, 4);
	gotoxy(x, y++);
	cprintf("   �   �    �   ��%c %c��  ����Ĵ �    � �����  ������  ����Ŀ  ", 4, 4);
...
char user_input()
{
	...
	if (kbhit())
	{
		...
		case 0:
			...
			case DOWN:
				Ships.Player->_ChangeVelocityx(0);
				break;
			} //End of inner switch
			break;

		case SPACE:
			Bullets.bullet_shot(bullet_mode, Ships.Player->getx(), 20);
			break;
...
void initialize_level(char difficulty, int &spawn_rate, char &max_no_ships)
{
	...
	case 9:
		spawn_rate = 50; //will not survive for long
	...
	while (flag)
	{
		...
		if (kbhit())
		{
			...
			case 0:
				...
				case DOWN:
					if (f == item_number - 1) //bottom position
						f = 0;
					else
						f++;
					break;
